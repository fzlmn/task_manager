...
interface PageProps {
  params?: Promise<string>; // Ensure the `params` type matches the Promise type
...
function Page({ params }: PageProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Ensure the resetToken is available from the params or context
    if (params) {
      params.then((resetToken) => {
  ...
  return (
    <main className="auth-page w-full h-full flex justify-center items-center">
      <form className="m-[2rem] px-10 py-14 rounded-lg bg-white max-w-[520px] w-full">
        ...
        <Image src="/flurry.png" alt="Flurry" />
      </form>
    </main>
  );
}
